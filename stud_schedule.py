...
class TeacherScheduleConversation(CommonConversation):
    ...
    def choice_teacher(self, update: Update, context: CallbackContext) -> int:
        ...
        context.user_data['teacher'] = text
    ...
    def choice_teacher_entry(self, update: Update, _: CallbackContext) -> int:
        update.message.reply_text('Напиши ФИО преподавателя')
        return self.CHOICE_GROUP
...
def main() -> None:
    ...
    choice_group_conv_handler = ConversationHandler(
        entry_points=[CommandHandler('choice_group', tsc.choice_group_entry)],
        states={
            tsc.CHOICE_GROUP: [
                MessageHandler(Filters.text, tsc.choice_group),
            ],
            tsc.CONFIRM_GROUP: [
                MessageHandler(
                    Filters.text & ~(Filters.command | Filters.regex('^Done$')), tsc.confirm_group
                )
            ],
            tsc.TRY_AGAIN: [
                MessageHandler(
                    Filters.text & ~(Filters.command | Filters.regex('^Done$')), tsc.try_again,
                )
            ],
        },
